        ...
        sshagent(credentials: ["$SSH_CREDENTIALS_ID"]) {
          script { 
            sh '''
             ...
             ssh ${PRODUCTION_USER}@${PRODUCTION_SERVER} -o RemoteCommand="docker stop calc || true && docker rm calc || true && docker run --name calc -d -p 5000:5000 992382545251.dkr.ecr.us-east-1.amazonaws.com/dw-cicd-exam/calculator:latest"
               '''
          }
   }
