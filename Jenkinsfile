    ...
    stage('Deploy to production server'){
      ...
      steps {
        sshagent(credentials: ["$SSH_CREDENTIALS_ID"]) {
          ...
          ssh -o 'dcoker stop calc; docker run --name calc -d -p 5000:5000 992382545251.dkr.ecr.us-east-1.amazonaws.com/dw-cicd-exam/calculator:latest' $PRODUCTION_USER@$PRODUCTION_SERVER 
             '''
          }
   }
